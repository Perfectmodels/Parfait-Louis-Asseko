...
const generateArticleHtml = (article: Article, siteConfig: any): string => {
    const renderContentHtml = (content: ArticleContent): string => {
        switch (content.type) {
            ...
            case 'image': return `<figure style="margin: 24px 0;"><img src="${content.src}" alt="${escapeHtml(content.alt)}" style="width: 100%; height: auto; border-radius: 4px;" />${content.caption ? `<figcaption style="font-size: 12px; text-align: center; color: #777; margin-top: 8px;">${escapeHtml(content.caption)}</figcaption>` : ''}</figure>`;
            default: return '';
        }
    };
    ...
    return `
        ...
        <body>
            <div class="page">
                ...
                <img src="${article.imageUrl}" alt="${escapeHtml(article.title)}" class="main-image" />
