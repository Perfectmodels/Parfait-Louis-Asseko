...
const generateArticleHtml = (article: Article, siteConfig: any): string => {
    const renderContentHtml = (content: ArticleContent): string => {
        const text = escapeHtml('text' in content ? content.text : '');
        switch (content.type) {
            case 'heading': return `<h2 style="font-family: 'Times New Roman', Times, serif; font-size: 22px; color: #D4AF37; margin: 24px 0 12px 0; font-weight: bold;">${text}</h2>`;
            case 'paragraph': return `<p style="margin-bottom: 1em; line-height: 1.6;">${text}</p>`;
            case 'quote': return `<blockquote style="margin: 1.5em 0; padding: 1em; border-left: 4px solid #D4AF37; background-color: #f9f9f9; font-style: italic;"><p>"${text}"</p>${content.author ? `<cite style="display: block; text-align: right; margin-top: 0.5em; font-style: normal;">— ${escapeHtml(content.author)}</cite>` : ''}</blockquote>`;
