...
const generateArticleHtml = (article: Article, siteConfig: any): string => {
    ...
    const safeAuthor = escapeHtml(article.author || 'La Rédaction');
    const safeImageUrl = escapeHtml(article.imageUrl);
    const safeLogoUrl = siteConfig?.logo ? escapeHtml(siteConfig.logo) : '';
            ...
            <div class="page">
                <header class="header">
                    <div>
                        ...
                        <h1>${safeTitle}</h1>
                        <p class="meta">Par ${safeAuthor} • ${new Date(article.date).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
                    </div>
                     ${safeLogoUrl ? `<img src="${safeLogoUrl}" alt="Logo" />` : ''}
                </header>
                <img src="${safeImageUrl}" alt="${safeTitle}" class="main-image" />
