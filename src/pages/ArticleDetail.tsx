...
const generateArticleHtml = (article: Article, siteConfig: any): string => {
    const renderContentHtml = (content: ArticleContent): string => {
        switch (content.type) {
            case 'heading': return `<h2 style="font-family: 'Times New Roman', Times, serif; font-size: 22px; color: #D4AF37; margin: 24px 0 12px 0; font-weight: bold;">${escapeHtml('text' in content ? content.text : '')}</h2>`;
            case 'paragraph': return `<p style="margin-bottom: 1em; line-height: 1.6;">${escapeHtml('text' in content ? content.text : '')}</p>`;
            case 'quote': return `<blockquote style="margin: 1.5em 0; padding: 1em; border-left: 4px solid #D4AF37; background-color: #f9f9f9; font-style: italic;"><p>"${escapeHtml('text' in content ? content.text : '')}"</p>${'author' in content && content.author ? `<cite style="display: block; text-align: right; margin-top: 0.5em; font-style: normal;">— ${escapeHtml(content.author)}</cite>` : ''}</blockquote>`;
            case 'image': return `<figure style="margin: 24px 0;"><img src="${escapeHtml('src' in content ? content.src : '')}" alt="${escapeHtml('alt' in content ? content.alt : '')}" style="width: 100%; height: auto; border-radius: 4px;" />${'caption' in content && content.caption ? `<figcaption style="font-size: 12px; text-align: center; color: #777; margin-top: 8px;">${escapeHtml(content.caption)}</figcaption>` : ''}</figure>`;
            default: return '';
        }
    };
