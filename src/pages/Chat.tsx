...
const Chat: React.FC = () => {
  ...
  useEffect(() => {
    const initChat = async () => {
      try {
        ...
        setMessages([{ sender: 'ai', text: response.text || 'Bonjour ! Je suis PMM Assistant, comment puis-je vous aider ?' }]);
      } catch (err: any) {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
        ...
        const aiMessage: Message = { sender: 'ai', text: response.text || "Je n'ai pas pu générer de réponse. Veuillez réessayer.", sources: sources.length > 0 ? sources : undefined };
