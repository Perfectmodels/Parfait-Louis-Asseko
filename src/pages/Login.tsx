...
const Login: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (!window.crypto || !window.crypto.subtle) {
      setError('Environnement non sécurisé : cryptographie non disponible.');
      return;
    }
    const encoder = new TextEncoder();
    const dataBuf = encoder.encode(password);
    const hashBuffer = await window.crypto.subtle.digest('SHA-256', dataBuf);
    ...
    const foundUser = users.find(u => 
        (
            ...
            (u.type === 'admin' && u.user.password === hashedPassword) ||
            (u.type !== 'admin' && u.user.password === password)
        )
    );
