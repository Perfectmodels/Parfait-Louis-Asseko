...
const Login: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // Hash password for admin check if window.crypto is available
    ...
    if (window.crypto && window.crypto.subtle) {
        ...
        try {
            ...
            hashedInput = '$sha256$' + hashHex;
        } catch (err) {
            console.error('Erreur hachage mot de passe:', err);
        }
    }

    const adminProfile = data.adminProfile || { name: 'Admin', username: 'admin', password: '$sha256$0e89f223e226ae63268cf39152ab75722e811b89d29efb22a852f1667bd22ae0' };
    const adminPasswordMatches = adminProfile.password?.startsWith('$sha256$')
        ? adminProfile.password === hashedInput
        : adminProfile.password === password;

    const users = [
        ...(adminPasswordMatches ? [{ type: 'admin', user: adminProfile, path: '/admin' }] : []),
    ...
    const foundUser = users.find(u => {
        if (u.type === 'admin') {
           return ('username' in u.user && u.user.username?.toLowerCase() === normalizedUsername) ||
