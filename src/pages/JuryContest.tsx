...
export default function JuryContest() {
  ...
  // ⚡ Bolt Optimization: Memoize O(1) score map to prevent O(N³) rendering bottleneck
  // previously `.find()` was called for every candidate x passage combination
  ...
  const getScore = (candidateId: string, passageId: string) =>
    scoreMap.get(`${candidateId}-${passageId}`);
