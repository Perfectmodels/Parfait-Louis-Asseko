...
export default function JuryContest() {
  ...
  // ⚡ Bolt Optimization: Precompute score dictionary to avoid O(N^3) find operations during render
  const scoreDictionary = useMemo(() => {
    const dict = new Map<string, Score>();
    scores.forEach(s => {
      dict.set(`${s.candidateId}-${s.passageId}`, s);
    });
    return dict;
  }, [scores]);

  const getScore = (candidateId: string, passageId: string) =>
    scoreDictionary.get(`${candidateId}-${passageId}`);
