...
export default function JuryContest() {
  ...
  const criteriaForPassageMap = useMemo(() => {
    const map = new Map<string, ScoringCriteria[]>();
    effectivePassages.forEach(p => {
      map.set(p.id, criteria.filter(c => !c.passageId || c.passageId === p.id));
    });
    return map;
  }, [criteria, effectivePassages]);

  const criteriaForPassage = (passageId: string) => criteriaForPassageMap.get(passageId) || [];
