import React from 'react';
import { Link } from 'react-router-dom';
import CountdownTimer from '../components/CountdownTimer';
import { CalendarDaysIcon, ClockIcon, MapPinIcon, CheckCircleIcon } from '@heroicons/react/24/outline';
import SEO from '../components/SEO';
import { useData } from '../contexts/DataContext';
import { motion } from 'framer-motion';

const Casting: React.FC = () => {
  const { data, isInitialized } = useData();
  const castingDate = "2025-09-06T14:00:00";
  
  if (!isInitialized || !data) {
    return <div className="h-screen bg-pm-dark flex items-center justify-center"><div className="w-12 h-px bg-pm-gold animate-pulse"></div></div>;
  }
  
  const { siteImages } = data;

  return (
    <div className="bg-pm-dark text-pm-off-white overflow-x-hidden">
      <SEO title="Casting | Rejoindre PMM" description="Casting de mannequins à Libreville, Gabon." />

      {/* HERO SECTION */}
      <section className="relative h-screen flex items-center justify-center overflow-hidden">
        <motion.div 
          initial={{ scale: 1.1 }}
          animate={{ scale: 1 }}
          transition={{ duration: 2.5 }}
          className="absolute inset-0 bg-cover bg-center grayscale-[0.8] brightness-[0.4]"
          style={{ backgroundImage: `url('${siteImages.castingBg}')` }}
        />
        <div className="absolute inset-0 bg-gradient-to-b from-transparent via-pm-dark/80 to-pm-dark"></div>
        
        <div className="relative z-10 text-center px-6 space-y-12">
           <motion.span 
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="section-label"
           >
              Tournée Nationale de Scouting 2025
           </motion.span>
           <motion.h1 
              initial={{ opacity: 0, y: 30 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.5, duration: 1.2 }}
              className="text-5xl md:text-8xl font-playfair font-black text-white leading-tight tracking-tighter"
           >
              Trouve ton <br/><span className="italic gold-gradient-text">Moment</span>
           </motion.h1>
           <motion.p 
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              transition={{ delay: 1 }}
              className="max-w-2xl mx-auto text-xl font-light italic text-white/50"
           >
              "Nous ne cherchons pas la beauté, nous cherchons la présence. Nous ne trouvons pas des visages, nous trouvons des icônes."
           </motion.p>
        </div>

        <div className="absolute bottom-0 left-0 w-full p-12 lg:p-20 z-20 flex flex-col md:flex-row justify-between items-end gap-12">
            <div className="flex gap-12 border-l border-pm-gold/30 pl-12">
                <InfoItem icon={CalendarDaysIcon} label="Quand" value="6 Sept 2025" />
                <InfoItem icon={MapPinIcon} label="Où" value="Libreville, Gabon" />
            </div>
            <div className="scale-75 md:scale-100 origin-right">
                <CountdownTimer targetDate={castingDate} />
            </div>
        </div>
      </section>

      {/* REQUIREMENTS SECTION */}
      <section className="page-container grid grid-cols-1 lg:grid-cols-2 gap-32 items-start">
          <div className="space-y-20">
              <div className="space-y-8">
                  <span className="section-label">Critères</span>
                  <h2 className="text-4xl md:text-6xl font-playfair font-black leading-tight italic">Qui recherchons-nous ?</h2>
                  <p className="text-xl text-white/40 leading-relaxed font-light">La diversité est notre force. Nous recrutons des talents pour le défilé, l'éditorial et la publicité.</p>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-16">
                  <div className="space-y-6">
                      <h3 className="text-[10px] font-black uppercase tracking-[0.4em] text-pm-gold pb-4 border-b border-white/10">Critères Femme</h3>
                      <ul className="space-y-4 text-sm font-bold text-white/60">
                          <li>16 - 28 ans</li>
                          <li>Minimum 170cm</li>
                          <li>Tour de taille : 60 - 66cm</li>
                          <li>Tour de hanches : 90 - 96cm</li>
                      </ul>
                  </div>
                  <div className="space-y-6">
                      <h3 className="text-[10px] font-black uppercase tracking-[0.4em] text-pm-gold pb-4 border-b border-white/10">Critères Homme</h3>
                      <ul className="space-y-4 text-sm font-bold text-white/60">
                          <li>18 - 30 ans</li>
                          <li>Minimum 180cm</li>
                          <li>Silhouette athlétique</li>
                      </ul>
                  </div>
              </div>
          </div>

          <div className="glass-card p-16 space-y-12">
              <h3 className="text-3xl font-playfair font-black italic">Le Code Vestimentaire</h3>
              <p className="text-sm text-white/40 leading-relaxed">Venez prêt(e) à travailler. Sans maquillage, coiffure simple.</p>
              <div className="space-y-8">
                  <CheckListItem text="Haut noir simple" />
                  <CheckListItem text="Jean slim noir" />
                  <CheckListItem text="Talons (Femme) / Chaussures habillées (Homme)" />
                  <CheckListItem text="Peau nette & cheveux naturels" />
              </div>
              <div className="pt-12">
                  <Link to="/casting-formulaire" className="btn-premium w-full !py-6">Commencer la Pré-sélection</Link>
              </div>
          </div>
      </section>

      {/* CTA BOTTOM */}
      <section className="py-40 bg-white text-pm-dark text-center overflow-hidden relative">
          <div className="relative z-10 space-y-12 max-w-5xl mx-auto px-6">
             <h2 className="text-5xl md:text-8xl font-playfair font-black italic">C'est ton moment de briller.</h2>
             <p className="text-xl font-bold uppercase tracking-[0.3em] opacity-40">Postule aujourd'hui et rejoins l'élite.</p>
             <Link to="/casting-formulaire" className="btn-premium !bg-pm-dark !text-pm-gold !border-none !px-20 !py-8 text-sm">Postuler Maintenant</Link>
          </div>
          <div className="absolute -top-10 -left-10 text-[20rem] font-playfair font-black text-black/[0.03] select-none pointer-events-none">2025</div>
      </section>
    </div>
  );
};

const InfoItem: React.FC<{ icon: React.ElementType, label: string, value: string }> = ({ icon: Icon, label, value }) => (
    <div className="space-y-2">
        <span className="text-[10px] uppercase tracking-[0.4em] font-black text-white/30">{label}</span>
        <div className="flex items-center gap-3">
            <Icon className="w-5 h-5 text-pm-gold" />
            <span className="text-xl font-playfair font-bold text-white">{value}</span>
        </div>
    </div>
);

const CheckListItem: React.FC<{ text: string }> = ({ text }) => (
    <div className="flex items-center gap-4">
        <div className="w-6 h-6 rounded-full border border-pm-gold/30 flex items-center justify-center shrink-0">
            <CheckCircleIcon className="w-4 h-4 text-pm-gold" />
        </div>
        <span className="text-sm font-bold uppercase tracking-widest text-white/80">{text}</span>
    </div>
);

export default Casting;