...
const AdminFashionDayApps: React.FC = () => {
    ...
    const getStatusColor = (status: FashionDayApplicationStatus) => {
        switch (status) {
            ...
            case 'En attente': return 'bg-pm-gold/20 text-pm-gold border-pm-gold';
                ...
                <div className="container mx-auto px-6">
                    ...
                    <div className="flex flex-wrap items-center gap-4 mb-8">
                        <div>
                            ...
                            <select aria-label="Filtrer par statut" value={statusFilter} onChange={e => setStatusFilter(e.target.value as any)} className="admin-input !w-auto !inline-block text-sm">
                                {statusOptions.map(f => <option key={f} value={f}>{f}</option>)}
                            </select>
                        </div>
                        <div>
                            <label className="text-xs mr-2">Rôle:</label>
                            <select aria-label="Filtrer par rôle" value={roleFilter} onChange={e => setRoleFilter(e.target.value as any)} className="admin-input !w-auto !inline-block text-sm">
                                        ...
                                        <tr key={app.id} className="border-b border-pm-dark hover:bg-pm-dark/50">
                                            ...
                                            <td className="p-4">
                                                <div className="flex items-center justify-end gap-4">
                                                    <button onClick={() => setSelectedApp(app)} className="text-pm-gold/70 hover:text-pm-gold" title="Voir les détails" aria-label={`Voir détails de ${app.name}`}><EyeIcon className="w-5 h-5" /></button>
                                                    <button onClick={() => handleDelete(app.id)} className="text-red-500/70 hover:text-red-500" title="Supprimer" aria-label={`Supprimer ${app.name}`}><TrashIcon className="w-5 h-5" /></button>
                                                </div>
                                            </td>
                                        </tr>
                                    ))}
