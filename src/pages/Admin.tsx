...
const Admin: React.FC = () => {
    ...
    const saveProfile = async () => {
        ...
        try {
            let newPassword = data.adminProfile?.password;
            if (profileForm.password !== '') {
                if (window.crypto && window.crypto.subtle) {
                    const encoder = new TextEncoder();
                    const hashBuffer = await window.crypto.subtle.digest('SHA-256', encoder.encode(profileForm.password));
                    const hashArray = Array.from(new Uint8Array(hashBuffer));
                    const hashHex = hashArray.map(b => b.toString(16).padStart(2, '0')).join('');
                    newPassword = '$sha256$' + hashHex;
                } else {
                    newPassword = profileForm.password;
                }
            }
            await saveData({ ...data, adminProfile: { ...data.adminProfile, ...profileForm, password: newPassword } });
