...
const generateRegistrationListHtml = (applicants: CastingApplication[], siteConfig: any, contactInfo: any, title = 'Liste de Passage - Casting'): string => {
    const rows = applicants.map(app => `
        <tr style="border-bottom: 1px solid #eee;">
            <td style="padding: 8px; font-weight: bold; color: #D4AF37;">#${String(app.passageNumber || 0).padStart(3, '0')}</td>
            <td style="padding: 8px;">${app.firstName} ${app.lastName}</td>
            <td style="padding: 8px;">${app.gender}</td>
            <td style="padding: 8px;">${app.height || ''}</td>
            <td style="padding: 8px;">${app.weight || ''}</td>
            <td style="padding: 8px;">${app.chest || ''}</td>
            <td style="padding: 8px;">${app.waist || ''}</td>
            <td style="padding: 8px;">${app.hips || ''}</td>
            <td style="padding: 8px;">${app.shoeSize || ''}</td>
            ...
            <div class="sheet">
                ...
                <footer style="margin-top:24px; font-size:12px; color:#666;">
                  <div style="display:flex; gap:12px; align-items:center;">
                    <strong>Perfect Models Management</strong>
...
const RegistrationCasting: React.FC = () => {
    ...
    const registeredToday = registeredApplicants.filter(a => isSameDay(a.submissionDate));
    ...
    const handlePrint = () => {
        ...
        const html = generateRegistrationListHtml(registeredApplicants, data.siteConfig, data?.contactInfo, 'Liste de Passage - Casting (Complet)');
    ...
    const handlePrintToday = () => {
        if (!data?.siteConfig) return;
        const html = generateRegistrationListHtml(registeredToday, data.siteConfig, data?.contactInfo, 'Liste du Jour - Casting (Détails)');
    ...
    const handleDownloadCsvToday = () => {
        const headers = ['Passage','Nom','Genre','Taille','Poids','Poitrine','Taille (cm)','Hanches','Pointure','Téléphone','Heure'];
        const rows = registeredToday.map(a => [
            String(a.passageNumber || ''),
            ...
            a.shoeSize || '',
            a.phone || '',
            new Date(a.submissionDate).toLocaleTimeString('fr-FR')
        ]);
        ...
        a.href = url;
        a.download = `casting-liste-jour-${new Date().toISOString().slice(0,10)}.csv`;
                             ...
                             <div className="flex justify-between items-center mb-6">
                                ...
                                <div className="flex flex-wrap gap-2">
                                  <button onClick={handlePrint} className="print-hide inline-flex items-center justify-center gap-2 px-4 py-2 bg-pm-dark border border-pm-gold text-pm-gold font-bold uppercase tracking-widest text-sm rounded-full hover:bg-pm-gold hover:text-pm-dark">
                                      <PrinterIcon className="w-5 h-5"/> Imprimer (complet)
                                  </button>
                                  <button onClick={handlePrintToday} className="print-hide inline-flex items-center justify-center gap-2 px-4 py-2 bg-pm-dark border border-pm-gold text-pm-gold font-bold uppercase tracking-widest text-sm rounded-full hover:bg-pm-gold hover:text-pm-darka">
                                      <PrinterIcon className="w-5 h-5"/> Imprimer (jour)
                                  </button>
                                  <button onClick={handleDownloadCsvToday} className="print-hide inline-flex items-center justify-center gap-2 px-4 py-2 bg-black border border-pm-gold text-pm-gold font-bold uppercase tracking-widest text-sm rounded-full hover:bg-pm-gold/10">
