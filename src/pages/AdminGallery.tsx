...
const AdminGallery: React.FC = () => {
  ...
  const { albumMap, albumCounts, categoryCounts, noAlbumCount } = useMemo(() => {
    ...
    return { albumMap: map, albumCounts: aCounts, categoryCounts: cCounts, noAlbumCount: noAlbum };
  }, [albums, gallery]);
            ...
            <button key={tab} onClick={() => setActiveTab(tab as any)}
              ...
              <span className="ml-2 text-white/20">
                {tab === 'Tout' 
                  ...
                  : tab === 'Sans Album'
                    ? noAlbumCount
                    : categoryCounts.get(tab as GalleryCategory) || 0}
              </span>
            </button>
          ))}
        </div>
