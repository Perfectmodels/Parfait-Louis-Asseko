...
const AdminGallery: React.FC = () => {
  ...
  // Performance optimizations: precompute lookup maps to avoid O(N*M) rendering loops
  ...
  const { albumCounts, categoryCounts, noAlbumCount } = React.useMemo(() => {
    ...
    return { albumCounts: aCounts, categoryCounts: cCounts, noAlbumCount: noAlbum };
  }, [gallery]);
  ...
  const getAlbum = useCallback((item: GalleryItem) => {
    return item.albumId ? albumMap[item.albumId] : undefined;
  }, [albumMap]);
