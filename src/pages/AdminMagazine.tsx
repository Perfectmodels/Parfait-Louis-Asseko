...
const AdminMagazine: React.FC = () => {
    const { data, isInitialized, addDocument, updateDocument, deleteDocument } = useData();
    ...
    const handleFormSave = async (articleToSave: Article) => {
        try {
            if (isCreating) {
                ...
                // Use slug as ID if it was used as ID at creation. 
                // NOTE: In Firestore transition, we assume ID might be SLUG.
                await updateDocument('articles', articleToSave.slug, articleToSave);
            }
    ...
    const handleSetFeatured = async (slugToFeature: string) => {
        try {
            // First unfeature all others? Or just switch one? 
            // Existing logic: map all and set isFeatured.
            // With updateDocument, we can't map all easily.
            // We will just enable for this one. Ideally we should disable others but that requires batch.

            // Disable current featured one (opti: find it in localArticles)
            const currentFeatured = localArticles.find(a => a.isFeatured);
            if (currentFeatured && currentFeatured.slug !== slugToFeature) {
                await updateDocument('articles', currentFeatured.slug, { isFeatured: false });
            }

            await updateDocument('articles', slugToFeature, { isFeatured: true });
        } catch (error) {
