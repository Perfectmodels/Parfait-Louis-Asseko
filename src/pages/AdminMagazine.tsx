...
const AdminMagazine: React.FC = () => {
  const { data, saveData, isInitialized } = useDataStore();
  ...
  const handleStartCreate = () => {
    ...
    setEditingArticle({
      ...
      sponsorName: '',
      callToAction: { text: '', link: '', type: 'primary' }
    });
  };
            ...
            <div className="flex items-center gap-4">
                <button onClick={() => setIsGeneratorOpen(true)} className="action-btn-outline !flex !items-center !gap-2">
                 ...
                 <button onClick={handleStartCreate} className="action-btn !flex !items-center !gap-2">
                    <PlusCircleIcon className="w-5 h-5"/> Ajouter un Article
                </button>
            </div>
        </div>
        ...
        <div className="bg-pm-dark text-pm-off-white py-20 min-h-screen">
            <div className="container mx-auto px-6 max-w-3xl">
                ...
                <form onSubmit={handleSubmit} className="admin-section-wrapper space-y-6">
                    ...
                    <ImageUploader label="Image de couverture" value={formData.imageUrl} onChange={(value: string) => setFormData(p => ({...p, imageUrl: value}))} />
