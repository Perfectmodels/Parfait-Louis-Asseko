...
const AdminPayments: React.FC = () => {
  ...
  const handleStatusChange = async (id: string, newStatus: 'En attente' | 'Approuvé' | 'Rejeté') => {
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm('Supprimer cette soumission de paiement ?')) return;
  ...
  return (
    <AdminLayout>
      ...
      <AdminPageHeader
        ...
        subtitle="Gérez les paiements soumis par les mannequins."
      />

      <AdminSection title="Filtre">
      ...
      <AdminSection title="Soumissions">
        <AdminCard className="p-6">
                ...
                <div key={payment.id} className="bg-black p-6 border border-pm-gold/20 rounded-lg">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      ...
                      <p className="text-xs text-pm-off-white/50 mt-1">Soumis le {formatDate(payment.submissionDate)}</p>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className={`px-3 py-1 rounded-full text-xs font-medium border ${getStatusColor(payment.status)}`}>{payment.status}</span>
                      <button onClick={() => handleDelete(payment.id)} className="p-2 text-red-400 hover:bg-red-400/20 rounded-lg transition-colors" title="Supprimer">
                  ...
                  {payment.notes && (
                    ...
                    <div className="flex gap-2 pt-4 border-t border-pm-gold/20">
                      <button onClick={() => handleStatusChange(payment.id, 'Approuvé')} className="flex items-center gap-2 px-4 py-2 bg-green-600/20 text-green-400 border border-green-600/30 rounded-lg hover:bg-green-600/30 transition-colors">
                        <CheckIcon className="w-4 h-4" /> Approuver
                          </button>
                      <button onClick={() => handleStatusChange(payment.id, 'Rejeté')} className="flex items-center gap-2 px-4 py-2 bg-red-600/20 text-red-400 border border-red-600/30 rounded-lg hover:bg-red-600/30 transition-colors">
