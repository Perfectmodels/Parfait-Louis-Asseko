...
const NavLinkItem: React.FC<{ to: string; label: string; onClick?: () => void; isMobile?: boolean; isOpen?: boolean; delay?: number; }> = ({ to, label, onClick, isMobile = false, isOpen = false, delay = 0 }) => {
  ...
  return (
    <NavLink
      ...
      className={({ isActive }) =>
        `relative py-3 lg:py-2 text-pm-off-white uppercase text-sm tracking-widest transition-colors duration-300 group hover:text-pm-gold focus-style-self focus-visible:text-pm-gold ${mobileAnimationClasses} ` +
          ...
          <form onSubmit={handleMobileSearch} className="mb-8">
            <div className="relative">
              <input
                ...
                className="w-full bg-black border-2 border-pm-gold/50 rounded-full py-3 pl-10 pr-4 text-pm-off-white focus:outline-none focus:border-pm-gold transition-colors text-base"
              />
