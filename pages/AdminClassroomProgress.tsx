    ...
    return (
        <div className="bg-pm-dark text-pm-off-white py-20 min-h-screen">
            ...
            <div className="container mx-auto px-6">
                ...
                <h1 className="text-4xl font-playfair text-pm-gold mb-2">Suivi Pédagogique du Classroom</h1>
                <p className="text-pm-off-white/70 mb-8">Tableau récapitulatif des notes obtenues par chaque mannequin aux quiz.</p>
                        ...
                        <table className="w-full text-left">
                            <thead className="bg-pm-dark/50">
                                <tr className="border-b border-pm-gold/20">
                                    <th className="p-4 uppercase text-xs tracking-wider text-pm-off-white/70 sticky left-0 bg-pm-dark/50">Mannequin</th>
                                    {courseModulesWithQuizzes.map(module => (
                                        <th key={module.slug} className="p-4 uppercase text-xs tracking-wider text-pm-off-white/70 text-center whitespace-nowrap">
                                            {module.title.replace('Module ', 'M').split(':')[0]}
                                        </th>
                                    ))}
                                </tr>
                            </thead>
                                        ...
                                        {courseModulesWithQuizzes.map(module => {
                                            ...
                                            return (
                                                <td key={module.slug} className="p-4 text-center">
                                                    {score !== undefined ? (
                                                        <span className={`px-2 py-1 font-bold rounded ${getScoreColor(score)}`}>
                                                            {score}%
                                                        </span>
                                                    ) : (
                         ...
                         {models.length === 0 && <p className="text-center p-8 text-pm-off-white/60">Aucun mannequin à afficher.</p>}
                    </div>
                </div>
            </div>
        </div>
