...
Si votre navigateur est ouvert (même en arrière-plan), vous devriez recevoir la notification !

---

## 📂 Structure du Code

* `src/firebaseConfig.ts` : Initialisation des services (Analytics, Perf, Messaging).
* `public/sw.js` : Service Worker qui reçoit les notifications en arrière-plan.
* `src/hooks/useNotifications.ts` : Hook React pour gérer les permissions et le token.
* `src/components/NotificationTester.tsx` : Composant UI pour activer les notifs.
